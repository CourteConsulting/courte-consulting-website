...
const Hero: React.FC = () => {
  ...
  const scrollToFeaturedWebsites = () => {
    const element = document.getElementById('featured-websites');
    if (element) {
      element.scrollIntoView({ behavior: 'smooth' });
    }
  };
  ...
  return (
    <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="animate-fade-in-up">
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
            ...
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500">
              Websites, Marketing, and Product Development
            </span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            <button
              onClick={scrollToFeaturedWebsites}
              className="group bg-white/20 backdrop-blur-sm text-white px-8 py-4 rounded-full text-lg font-semibold hover:bg-white/30 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl"
            >
              See Our Work
              <ArrowDown className="inline-block ml-2 h-5 w-5 group-hover:translate-y-1 transition-transform" />
            </button>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
        <button onClick={scrollToContact} className="text-white/60 hover:text-white transition-colors">
